...
import { LuUser, LuCalendarDays, LuClock, LuChevronLeft, LuScissors, LuChevronRight, LuCoffee } from 'react-icons/lu';
...
const AgendarCita: React.FC = () => {
    ...
    if (peluqueroSel && fechaSel) {
      const consultarDisponibilidad = async () => {
        ...
        try {
          ...
          const data = await citasService.obtenerDisponibilidad(peluqueroSel.id, fechaString);
  ...
  const handleAgendar = async () => {
    ...
    try {
      ...
      presentToast({ message: '¡Cita agendada con éxito!', duration: 3000, color: 'success', position: 'top' });
      ...
      <IonContent scrollY={true}>
        ...
        >
          ...
          <div className="px-6 pb-36 space-y-10">
            ...
            <section>
              <div className="flex items-center gap-2 mb-5">
                <div
                  ...
                  style={{
                    background: paso >= 1 ? 'linear-gradient(135deg, #f59e0b, #d97706)' : 'rgba(255,255,255,0.05)',
                ...
                {peluqueros.map((pel) => {
                  ...
                  return (
                    <div
                      ...
                      style={{
                        background: seleccionado ? 'rgba(217,119,6,0.12)' : 'rgba(255,255,255,0.03)',
                        border: seleccionado ? '1.5px solid rgba(217,119,6,0.5)' : '1.5px solid rgba(255,255,255,0.06)',
                        boxShadow: seleccionado ? '0 4px 20px rgba(217,119,6,0.15)' : 'none',
                      }}
                    >
                      <div
                        className="w-12 h-12 rounded-2xl mx-auto mb-3 flex items-center justify-center"
                        style={{
                          background: seleccionado ? 'rgba(217,119,6,0.2)' : 'rgba(255,255,255,0.05)',
                        }}
                      >
                        <LuScissors className={`text-xl ${seleccionado ? 'text-amber-500' : 'text-zinc-600'}`} />
                      </div>
                      <p className={`font-bold text-xs leading-tight ${seleccionado ? 'text-amber-400' : 'text-zinc-400'}`}>
                        {pel.nombre}
                      </p>
                    </div>
                  );
            ...
            {/* PASO 2: Fecha */}
            <section className={!peluqueroSel ? 'opacity-30 pointer-events-none' : ''}>
              <div className="flex items-center gap-2 mb-5">
                <div
                  className="w-7 h-7 rounded-lg flex items-center justify-center text-xs font-black"
                  style={{
                    background: paso >= 2 ? 'linear-gradient(135deg, #f59e0b, #d97706)' : 'rgba(255,255,255,0.05)',
              ...
              <div className="flex gap-3 overflow-x-auto pb-2" style={{ scrollbarWidth: 'none' }}>
                {proximosDias.map((dia, idx) => {
                  ...
                  const seleccionado = fechaSel?.getDate() === dia.getDate() &&
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        boxShadow: seleccionado ? '0 4px 20px rgba(217,119,6,0.3)' : 'none',
                      }}
                    >
                      <p className={`text-xs font-bold uppercase mb-1 ${seleccionado ? 'text-black/60' : 'text-zinc-600'}`}>
                        {nombre}
                      </p>
                      <p className={`text-xl font-black ${seleccionado ? 'text-black' : 'text-white'}`}>
                        {numero}
                      </p>
                    </div>
                  );
              ...
              <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-2">
                  <div
                    className="w-7 h-7 rounded-lg flex items-center justify-center text-xs font-black"
                    style={{
                      background: paso >= 3 ? 'linear-gradient(135deg, #f59e0b, #d97706)' : 'rgba(255,255,255,0.05)',
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          boxShadow: seleccionada ? '0 4px 16px rgba(217,119,6,0.3)' : 'none',
                        }}
                      >
                        {hora}
                      </button>
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                boxShadow: puedeAgendar ? '0 8px 32px rgba(217,119,6,0.4)' : 'none',
              }}
            >
